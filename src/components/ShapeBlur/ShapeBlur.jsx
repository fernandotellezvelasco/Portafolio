...
void main() {
    ...
    // Compensate for the aspect-ratio correction in coord() so the shape's
    // width/height match the actual pixel width/height of the container
    // (instead of always looking square).
    vec2 aspectBox = u_resolution.x > u_resolution.y
    ...
    float sdf;
    if (VAR == 0) {
        sdf = sdRoundRect(st, vec2(size) * aspectBox, roundness);
